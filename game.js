...
// Добавляем переменную для хранения элементов счетчика
...
function createClickEffect(clientX, clientY, amount) {
    ...
    // Используем абсолютные координаты относительно окна
...
// Полная защита от сохранения и копирования изображений
function preventImageSaving() {
    // Блокируем контекстное меню на всём документе
...
function setupMultitouch() {
    ...
    // Анимация нажатия
    tapTarget.addEventListener('mousedown', () => {
        tapImage.style.transform = 'scale(0.95)';
    });
    
    tapTarget.addEventListener('mouseup', () => {
