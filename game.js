...
function formatNumber(num, forPerSecond = false) {
    if (forPerSecond) {
...
function updateGoldDisplay() {
    ...
    const perSecondText = formatNumber(goldPerSecond, true);
...
function setupMultitouch() {
    ...
    document.addEventListener('mouseup', handleTapEnd);
    ...
    tapTarget.addEventListener('touchstart', (e) => {
        ...
        if (x >= rect.left && x <= rect.right && y >= rect.top && y <= rect.bottom) {
            ...
            for (let i = 1; i < Math.min(e.touches.length, 3); i++) {
                const multiTouch = e.touches[i];
                if (multiTouch.clientX >= rect.left && multiTouch.clientX <= rect.right && 
                    multiTouch.clientY >= rect.top && multiTouch.clientY <= rect.bottom) {
                    gold += actualGoldPerClick;
                    createClickEffect(multiTouch.clientX, multiTouch.clientY, actualGoldPerClick);
                }
            }
        }
    }, { passive: false });
    ...
    tapTarget.addEventListener('touchend', (e) => {
        e.preventDefault();
        setTimeout(handleTapEnd, 50);
    }, { passive: false });
    
    tapTarget.addEventListener('touchcancel', handleTapEnd);
    
    tapTarget.addEventListener('contextmenu', (e) => {
        e.preventDefault();
        handleTapStart(e.clientX, e.clientY);
        setTimeout(handleTapEnd, 100);
        return false;
    });
}
...
perSecondBtn.addEventListener('touchstart', (e) => {
    e.preventDefault();
    openDrawer('perSecond');
}, { passive: false });

perClickBtn.addEventListener('click', () => openDrawer('perClick'));
perClickBtn.addEventListener('touchstart', (e) => {
    e.preventDefault();
    openDrawer('perClick');
}, { passive: false });
...
prestigeBtn.addEventListener('touchstart', (e) => {
    e.preventDefault();
    showPrestigeModal();
}, { passive: false });

confirmPrestige.addEventListener('click', confirmPrestigeAction);
confirmPrestige.addEventListener('touchstart', (e) => {
    e.preventDefault();
    confirmPrestigeAction();
}, { passive: false });

cancelPrestige.addEventListener('click', closePrestigeModal);
cancelPrestige.addEventListener('touchstart', (e) => {
    e.preventDefault();
    closePrestigeModal();
}, { passive: false });
...
prestigeModal.addEventListener('touchstart', (e) => {
